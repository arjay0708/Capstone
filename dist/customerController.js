...
function sendVerificationEmail(email, fname, verificationToken) {
...
router.post('/register', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [existingEmail] = yield pool.query('SELECT * FROM Accounts WHERE email = ?', [email]);
        if (existingEmail.length > 0) {
            return res.status(409).send('Email already in use');
        }
        const [existingUsername] = yield pool.query('SELECT * FROM Accounts WHERE username = ?', [username]);
        ...
        const [result] = yield pool.query('INSERT INTO Accounts (username, password, email, fname, lname, address, age, phone, verification_token, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [username, hashedPassword, email, fname, lname, address, age, phone, verificationToken, false]);
        if (result.affectedRows > 0) {
            // Send verification email with a link containing the token
            yield sendVerificationEmail(email, fname, verificationToken);
...
router.get('/verify-email/:token', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [user] = yield pool.query('SELECT * FROM Accounts WHERE verification_token = ?', [token]);
        if (user.length === 0) {
        ...
        const [updateResult] = yield pool.query('UPDATE Accounts SET is_verified = ?, verification_token = NULL WHERE verification_token = ?', [true, token]);
...
router.post('/login', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [results] = yield pool.query('SELECT * FROM Accounts WHERE username = ? AND role = "customer"', [username]);
        if (results.length === 0) {
...
router.get('/buyer-details', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [results] = yield pool.query('SELECT fname, lname, mname, address, phone FROM Accounts WHERE account_id = ?', [account_id]);
        if (results.length === 0) {
...
router.get('/user-orders/:accountId', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const ordersWithItems = yield Promise.all(orders.map((order) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            `, [order.order_id]);
            const itemsWithImages = orderItems.map(item => (Object.assign(Object.assign({}, item), { images: JSON.parse(item.images).map(image => `/uploads/${image}`), price_at_purchase: parseFloat(item.price_at_purchase) })));
            return Object.assign(Object.assign({}, order), { items: itemsWithImages });
        })));
...
router.put('/edit-profile', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [userResult] = yield pool.query('SELECT * FROM Accounts WHERE account_id = ?', [accountId]);
        ...
        const [result] = yield pool.query(updateQuery, params);
        if (result.affectedRows > 0) {
