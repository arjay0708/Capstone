...
router.post('/add-to-cart', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [variant] = yield pool.query('SELECT quantity AS available_quantity FROM ProductVariant WHERE variant_id = ?', [product_variant_id]);
        if (variant.length === 0) {
        ...
        let [cart] = yield pool.query('SELECT * FROM Cart WHERE account_id = ?', [account_id]);
        if (cart.length === 0) {
            // Create a new cart if none exists
            const [newCart] = yield pool.query('INSERT INTO Cart (account_id) VALUES (?)', [account_id]);
        ...
        const [existingItem] = yield pool.query('SELECT * FROM CartItem WHERE cart_id = ? AND product_variant_id = ?', [cart.cart_id, product_variant_id]);
        if (existingItem.length > 0) {
            ...
            yield pool.query('UPDATE CartItem SET quantity = ? WHERE cart_id = ? AND product_variant_id = ?', [newQuantity, cart.cart_id, product_variant_id]);
        }
        else {
            ...
            yield pool.query('INSERT INTO CartItem (cart_id, product_variant_id, quantity) VALUES (?, ?, ?)', [cart.cart_id, product_variant_id, quantity]);
        }
...
router.get('/view', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const account_id = req.user.account_id; // Retrieved from the middleware
    try {
        // Retrieve cart by account ID
        const [cart] = yield pool.query('SELECT * FROM Cart WHERE account_id = ?', [account_id]);
        if (cart.length === 0) {
        ...
        const [items] = yield pool.query(`SELECT 
        ...
        const updatedItems = items.map(item => {
            ...
            return Object.assign(Object.assign({}, item), { images });
        });
        res.status(200).json({
            cart_id,
            account_id,
...
router.post('/create-order', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [cartItems] = yield pool.query(`SELECT 
                CartItem.cart_item_id,
                CartItem.product_variant_id,
             ...
             WHERE CartItem.cart_item_id IN (?) AND Cart.account_id = ?`, [cart_item_ids, account_id]);
        ...
        const [newOrder] = yield pool.query('INSERT INTO Orders (account_id, total_amount) VALUES (?, ?)', [account_id, totalAmount]);
        const order_id = newOrder.insertId;
        for (const item of cartItems) {
            yield pool.query('INSERT INTO OrderItem (order_id, product_variant_id, quantity, price_at_purchase) VALUES (?, ?, ?, ?)', [order_id, item.product_variant_id, item.order_quantity, item.price]);
            yield pool.query('UPDATE ProductVariant SET quantity = quantity - ? WHERE variant_id = ?', [item.order_quantity, item.product_variant_id]);
        }
        // Remove only the selected items from the cart
        yield pool.query('DELETE FROM CartItem WHERE cart_item_id IN (?)', [cart_item_ids]);
        res.status(200).json({
...
router.delete('/remove/:cart_item_id', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { cart_item_id } = req.params;
    const account_id = req.user.account_id;
    try {
        // Verify the cart item belongs to the user's cart
        const [cartItem] = yield pool.query(`SELECT CartItem.cart_item_id 
             FROM CartItem 
             JOIN Cart ON CartItem.cart_id = Cart.cart_id 
             WHERE CartItem.cart_item_id = ? AND Cart.account_id = ?`, [cart_item_id, account_id]);
        if (cartItem.length === 0) {
            return res.status(404).json({ message: 'Cart item not found or does not belong to this account' });
        }
        // Delete the cart item
        yield pool.query('DELETE FROM CartItem WHERE cart_item_id = ?', [cart_item_id]);
