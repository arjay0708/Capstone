...
router.post('/', upload.array('images'), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [productResult] = yield connection.query('INSERT INTO Product (Pname, price, images, category, description) VALUES (?, ?, ?, ?, ?)', [Pname, price, imagesJson, category, description]);
        ...
        yield Promise.all(variantQueries);
...
router.put('/update/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        yield connection.query('UPDATE Product SET price = ? WHERE product_id = ?', [price, productId]);
        // Update product variants' quantities
        for (const variant of variants) {
            yield connection.query('UPDATE ProductVariant SET quantity = ? WHERE product_id = ? AND size = ?', [variant.quantity, productId, variant.size]);
        }
        yield connection.commit();
        connection.release();
...
router.get('/latest', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        LIMIT 4
      `;
        const [latestProducts] = yield pool.query(query);
        // Parse image URLs
...
router.delete('/delete/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const productId = req.params.id;
    const connection = yield pool.getConnection().catch(err => {
        return res.status(500).json({ error: 'DB connection error' });
    });
    try {
        yield connection.beginTransaction();
        yield connection.query('DELETE FROM ProductVariant WHERE product_id = ?', [productId]);
        yield connection.query('DELETE FROM Product WHERE product_id = ?', [productId]);
        yield connection.commit();
...
router.get('/filter', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const connection = yield pool.getConnection();
        const [products] = yield connection.query(productQuery, productParams);
        const [variantResults] = yield connection.query(variantQuery, variantParams);
        connection.release();
...
router.get('/search', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const connection = yield pool.getConnection();
        const [results] = yield connection.query('SELECT * FROM Product WHERE Pname LIKE ?', [`%${search}%`]);
        connection.release();
        if (results.length === 0) {
            return res.status(404).json({ error: 'No products found' });
        }
...
router.get('/category-counts', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const query = `
...
router.get('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        connection = yield pool.getConnection();
        const [results] = yield connection.query(query, [productId]);
        if (results.length === 0) {
            return res.status(404).json({ error: 'Product not found' });
        }
...
router.get('/orders/all', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Fetch all orders and join with user details, including order status
        const [orders] = yield pool.query(`
            SELECT 
                Orders.*,
        ...
        const [orderItems] = yield pool.query(`SELECT 
             ...
             ORDER BY OrderItem.order_id`, [orderIds]);
        // Organize items under their respective orders
        const ordersWithItems = orders.map(order => (Object.assign(Object.assign({}, order), { customer_name: order.customer_name, customer_email: order.customer_email, customer_phone: order.customer_phone, customer_address: order.customer_address, items: orderItems
                .filter(item => item.order_id === order.order_id)
                .map(item => (Object.assign(Object.assign({}, item), { images: JSON.parse(item.images).map(image => `/uploads/${image}`), price_at_purchase: parseFloat(item.price_at_purchase) }))) })));
...
router.put('/ship-order/:order_id', authMiddleware, roleCheckMiddleware(['admin', 'employee']), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const [order] = yield pool.query('SELECT * FROM Orders WHERE order_id = ?', [order_id]);
        if (order.length === 0) {
        ...
        yield pool.query('UPDATE Orders SET order_status = ?, tracking_number = ?, carrier = ?, shipped_at = NOW() WHERE order_id = ?', ['Shipped', tracking_number, carrier, order_id]);
        // Schedule auto-update to "Delivered"
        setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
            try {
                yield pool.query('UPDATE Orders SET order_status = ?, delivered_at = NOW() WHERE order_id = ? AND order_status = ?', ['Delivered', order_id, 'Shipped']);
...
router.put('/deliver-order/:order_id', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { order_id } = req.params;
    try {
        ...
        yield pool.query('UPDATE Orders SET order_status = ?, delivered_at = NOW() WHERE order_id = ?', ['Delivered', order_id]);
...
router.get('/orders', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const account_id = req.user.account_id;
    try {
        const [orders] = yield pool.query('SELECT * FROM Orders WHERE account_id = ?', [account_id]);
...
router.get('/order/:id', authMiddleware, roleCheckMiddleware(['admin', 'employee']), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const account_id = req.user.account_id;
    const order_id = req.params.id;
    try {
        // Check if the user is admin/employee, and allow them to see any order
        if (req.user.role === 'admin' || req.user.role === 'employee') {
            const [order] = yield pool.query(`
                SELECT 
                    Orders.*,
                    CONCAT(accounts.fname, ' ', accounts.lname) AS name,
                    accounts.email,
            ...
            const [orderItems] = yield pool.query(`
                SELECT 
                    OrderItem.order_item_id,
                    OrderItem.product_variant_id,
                    OrderItem.quantity,
            ...
            const itemsWithImages = orderItems.map(item => (Object.assign(Object.assign({}, item), { images: JSON.parse(item.images).map(image => `/uploads/${image}`), price_at_purchase: parseFloat(item.price_at_purchase) // Ensure it's a number
             })));
            // Send the response
...
router.put('/order/:id/status', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const order_id = req.params.id;
    const { status } = req.body; // Expect status to be one of 'Pending', 'Shipped', 'Delivered', 'Cancelled'
    try {
        const [result] = yield pool.query('UPDATE Orders SET order_status = ? WHERE order_id = ?', [status, order_id]);
...
router.put('/cancel/:id/status', authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [order] = yield pool.query('SELECT order_status FROM Orders WHERE order_id = ?', [order_id]);
        if (order.length === 0) {
            return res.status(404).json({ message: 'Order not found' });
        }
        ...
        const [result] = yield pool.query('UPDATE Orders SET order_status = ? WHERE order_id = ?', ['Cancelled', order_id]);
        if (result.affectedRows === 0) {
...
router.get('/sales/sales', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    ...
    try {
        const [salesToday] = yield pool.query('SELECT SUM(total_amount) AS total FROM orders WHERE delivered_at >= ?', [todayStart]);
        const [salesWeekly] = yield pool.query('SELECT SUM(total_amount) AS total FROM orders WHERE delivered_at >= ?', [weekStart]);
        const [salesMonthly] = yield pool.query('SELECT SUM(total_amount) AS total FROM orders WHERE delivered_at >= ?', [monthStart]);
        res.status(200).json({
            salesToday: ((_a = salesToday[0]) === null || _a === void 0 ? void 0 : _a.total) || 0,
            salesWeekly: ((_b = salesWeekly[0]) === null || _b === void 0 ? void 0 : _b.total) || 0,
            salesMonthly: ((_c = salesMonthly[0]) === null || _c === void 0 ? void 0 : _c.total) || 0
        });
    }
    catch (err) {
